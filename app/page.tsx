...
export default function App() {
  ...
  return (
   ...
   <button onClick={() => SetRecChoos({ ...RecChoos, budget: !RecChoos.budget })}> ความคุ้มค่า/งบประหยัด {RecChoos.budget ? "✓" : "×"}</button>
        ...
        <div className="mt-4">
          <button className="bg-blue-600 text-white p-2 px-4 rounded" onClick={() => SetfindButton(!getfindButton) }>
            <span>ค้นหา</span>
          </button>
        </div>
        
        <div>
        {PhoneArr.lenght >= 1 && PhoneArr.map((Data) => <Phonecard Device=Data/>) }
        </div>
...
ูก่อน --- */}
        <div className="mt-8">
          <h3 className="text-xl font-bold">ผลการค้นหา: {PhoneArr.length} รายการ</h3>
          <ul className="mt-4">
            {PhoneArr.map((phone, idx) => (
              <li key={idx} className="border p-2 mb-2">{phone.name}</li>
            ))}
          </ul>
        </div>
      </main>
