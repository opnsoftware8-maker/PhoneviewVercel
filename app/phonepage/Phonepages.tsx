...
ปิด
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-5">
        ...
        <section>
          ...
          <p className="text-zinc-700 bg-zinc-50 px-4 py-3 rounded-xl border border-zinc-100">
            ...
            <span itemProp="description">{phoneDat.chip}</span>
          </p>
        </section>
        ...
        <section>
          ...
          <div className="bg-zinc-50 px-4 py-3 rounded-xl border border-zinc-100">
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 text-zinc-700">
              {phoneDat.camera.main !== 0 && (
                <li>
                  <span className="font-semibold">กล้องหลัก:</span>{" "}
                  {phoneDat.camera.main} MP
                </li>
              )}

              {phoneDat.camera.ultrawide !== 0 && (
                <li>
                  <span className="font-semibold">กล้องมุมกว้าง:</span>{" "}
                  {phoneDat.camera.ultrawide} MP
                </li>
              )}

              {phoneDat.camera.telephoto !== 0 && (
                <li>
                  <span className="font-semibold">กล้อง Telephoto:</span>{" "}
                  {phoneDat.camera.telephoto} MP
                </li>
              )}

              {phoneDat.camera.selfie !== 0 && (
                <li>
                  <span className="font-semibold">กล้องหน้า:</span>{" "}
                  {phoneDat.camera.selfie} MP
                </li>
              )}

              {phoneDat.camera.opticalZoom &&
                phoneDat.camera.opticalZoom !== 0 && (
                  <li>
                    <span className="font-semibold">ซูม Optical:</span>{" "}
                    {phoneDat.camera.opticalZoom}x
                  </li>
                )}

              {phoneDat.camera.digitalZoom !== 0 && (
                <li>
                  <span className="font-semibold">ซูม Digital:</span>{" "}
                  {phoneDat.camera.digitalZoom}x
                </li>
              )}
            </ul>
            ...
            <div className="mt-4 pt-3 border-t border-zinc-200">
              ...
              <div className="text-sm text-zinc-600 space-y-1">
                {phoneDat.camera.video !== "" && (
                  <p>คุณภาพ: {phoneDat.camera.video}</p>
                )}

                {phoneDat.camera.ois && (
                  <p>รองรับระบบกันสั่นทางฮาร์ดแวร์ (OIS)</p>
                )}
              </div>
            </div>
          </div>
        ...
        <section>
          ...
          <div className="bg-zinc-50 p-3 rounded-xl border border-zinc-100 flex gap-3 text-center">
            <div className="flex-1 bg-white p-2.5 rounded-lg shadow-sm border border-zinc-100">
              <p className="font-semibold text-zinc-500 text-xs mb-1">
                ความจุ
              </p>
              <p className="font-bold text-base">
                {phoneDat.battery} mAh
              </p>
            </div>
      ...
      <div className="space-y-5">
        ...
        >
          <h2 className="font-trirong text-xl font-bold mb-2">
            ราคาล่าสุด
          </h2>
          ...
          {phoneDat.latestPrice && (
            <>
              <h4 className="font-trirong text-3xl font-extrabold text-white mb-1">
                <span itemProp="price">
                  {phoneDat.latestPrice.defaultPrice}
                </span>{" "}
                บาท
              ...
              <p className="text-xs text-zinc-400 mb-4">
                อ้างอิงจาก {phoneDat.latestPrice.priceFrom}
              </p>
            </>
          )}
          ...
          <div className="space-y-2.5">
            {phoneDat.lazada && (
              <a
                href={phoneDat.lazada}
                target="_blank"
                rel="noopener noreferrer"
                className="block w-full text-center bg-blue-600 hover:bg-blue-500 transition py-2 rounded-xl font-bold text-sm"
              >
                สั่งซื้อบน Lazada
              </a>
            )}
            ...
            {phoneDat.shopee && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="block w-full text-center bg-orange-600 hover:bg-orange-500 transition py-2 rounded-xl font-bold text-sm"
              >
                สั่งซื้อบน Shopee
              </a>
            )}
        ...
        <section>
          <h2 className="font-trirong text-xl font-bold mb-3">
            แนะนำสำหรับ
          </h2>

          <div className="flex flex-wrap gap-2">
            {phoneDat.recommended.gaming && (
              <span className="bg-zinc-100 text-zinc-800 border border-zinc-200 px-3 py-1 rounded-lg text-sm font-medium">
                เล่นเกม
              </span>
            )}

            {phoneDat.recommended.camera && (
              <span className="bg-zinc-100 text-zinc-800 border border-zinc-200 px-3 py-1 rounded-lg text-sm font-medium">
                ถ่ายภาพ
              </span>
            )}

            {phoneDat.recommended.budget && (
              <span className="bg-zinc-100 text-zinc-800 border border-zinc-200 px-3 py-1 rounded-lg text-sm font-medium">
                งบประหยัด
              </span>
            )}

            {phoneDat.recommended.performance && (
              <span className="bg-zinc-100 text-zinc-800 border border-zinc-200 px-3 py-1 rounded-lg text-sm font-medium">
                ประสิทธิภาพ
              </span>
            )}

            {phoneDat.recommended.battery && (
              <span className="bg-zinc-100 text-zinc-800 border border-zinc-200 px-3 py-1 rounded-lg text-sm font-medium">
                แบตเตอรี่อึด
              </span>
            )}

            {phoneDat.recommended.entertainment && (
              <span className="bg-zinc-100 text-zinc-800 border border-zinc-200 px-3 py-1 rounded-lg text-sm font-medium">
                ความบันเทิง
              </span>
            )}
        ...
        <section className="bg-zinc-50 p-4 rounded-2xl border border-zinc-200">
          ...
          {phoneDat.pros.length > 0 && (
            <div className="mb-3">
              <h3 className="text-zinc-900 font-bold text-sm mb-1.5">
                ข้อดี:
              </h3>
          ...
          {phoneDat.cons.length > 0 && (
            <div>
              <h3 className="text-zinc-600 font-bold text-sm mb-1.5">
                ข้อสังเกต:
              </h3>
